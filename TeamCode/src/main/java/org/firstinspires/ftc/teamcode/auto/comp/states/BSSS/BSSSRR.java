...
@Autonomous(name="BSSSRR", group="BSSS")
public class BSSSRR extends SampleAuto {
    BaseRobot robot;
    int zone;
    @Override
    public void onInit() {
    ...
    public void onStart() {
        ...
        robot.drive.updatePoseEstimate();
        Actions.runBlocking(new ParallelAction(robot.autoGenerator.getStackToBackdropAutoAction(AutoUtil.RED, zone), new SequentialAction(robot.outtakeExcessPixels(), new SleepAction(2), robot.highOuttake())));
        Actions.runBlocking(robot.hopper.hopperOutake());
        robot.drive.updatePoseEstimate();
