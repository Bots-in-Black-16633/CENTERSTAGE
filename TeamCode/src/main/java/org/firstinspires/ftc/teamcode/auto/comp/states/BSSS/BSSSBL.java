package org.firstinspires.ftc.teamcode.auto.comp.states.BSSS;
...
@Autonomous(name="BSSSBL", group="BSSS")
public class BSSSBL extends SampleAuto {
    BaseRobot robot;
    int zone;
    @Override
    public void onInit() {
    ...
    public void onStart() {
        ...
        Actions.runBlocking(robot.hopper.hopperOutake());
        robot.drive.updatePoseEstimate();
        Actions.runBlocking(new ParallelAction(robot.autoGenerator.getBackdropToStackAutoAction(AutoUtil.BLUE), new SequentialAction(new SleepAction(.5), robot.resetToIntake())));
        Actions.runBlocking(robot.offTheTopStackIntake(1));
        robot.drive.updatePoseEstimate();
        Actions.runBlocking(new ParallelAction(robot.autoGenerator.getStackToBackdropAutoAction(AutoUtil.BLUE, zone), new SequentialAction(robot.outtakeExcessPixels(), new SleepAction(2), robot.highOuttake())));
        Actions.runBlocking(robot.hopper.hopperOutake());
        robot.drive.updatePoseEstimate();
